...
def _remove_zipmap(model: onnx.ModelProto) -> onnx.ModelProto:
    """onnxmltools emits a ZipMap that converts the probability tensor into a sequence
    of maps. onnxruntime_go cannot consume sequence outputs, so strip ZipMap and
    rewire to the raw float32 tensor.
    """
    graph = model.graph
...
def export_onnx(
    ...
    # onnxmltools requires feature names in 'f%d' form; rename booster internals temporarily.
